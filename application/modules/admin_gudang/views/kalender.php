...
<script>
    ...
    $hand_over_masuk = JSON.parse('<?= json_encode($data_hand_over_masuk);?>');
    $hand_over_keluar = JSON.parse('<?= json_encode($data_hand_over_keluar);?>');
    ...
    $.each($hand_over_keluar, function (i, v) {
        $data = {
            title           : 'HO IN - '+v.kode_hand_over,
    ...
    $.each($hand_over_masuk, function (i, v) {
        $data = {
            title           : 'HO OUT - '+v.kode_hand_over,
            start           : new Date(v.tanggal_dibuat),
            end             : new Date(v.tanggal_dibuat),
            allDay          : true,
            url             : "<?= base_url('asisten-manager-gudang/view-detail-hand-over/')?>"+v.kode_hand_over,
